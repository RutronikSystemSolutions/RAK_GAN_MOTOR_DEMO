...
#define ADC_CS_CURRENT_SENSE_POLARITY            (HS_Current_Sense)            /*Low side current sense "LS_Current_Sense" =1 or High side current Sense "HS_Current_Sense"=-1 TODO: add*/
/* TMR phase sensor (non-floating): inverted polarity due to sensor calibration
   ADC_Half_Point (2048 ticks) - ADC_Value = positive current
   Negative scale factor corrects the inversion */
...
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
/*********Current Controller*********/
#define MOTOR_CTRL_CURRENT_BW                      (1000.0f)                      /*[Hz], Current loop bandwidth*/
#define MOTOR_CTRL_CURRENT_FF_COEFF                (50.0f)                      /*[%], Current loop feed-forward coefficient*/
#define MOTOR_CTRL_CURRENT_STARTUP_THRESH          (MOTOR_CURRENT_CONT*0.10f)    /*[A], Current control startup threshold*/
#define MOTOR_CTRL_CURRENT_OPEN_LOOP_CMD           (MOTOR_CURRENT_CONT*0.20f)     /*[A], Current control open loop command value*/
